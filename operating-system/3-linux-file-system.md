...
- /mnt - temporary mount points
  - Definition
    ...
    - For manually mounting to the file system

## Interaction with these root folders

- Usually you are not interacting with these folders:
- When installing apps with package manager or installers or when interacting with Operating System, you are not directly updating these folders
  - Instead, the application installer, package manager or operating system is handling these
- for example
  - when plugging in a USB, it will automatically mounted in your system and you can see the folders/files on the usb in the /media without having to mount anything manually
  - when changing configs, there will be automatic updates on /etc
  - When installing apps, the package manager tools will handle to put the right files in the right location like /bin, /lib and track them, so when you clean the apps, the package manager will be remove them for humans.

## Hidden files

- Is primarily used to help prevent important data from being accidentally deleted, and for cleaner visual display of directories
- Automatically generated by programs or operating system
- File name starts with a dot. In UNIX also called "dotfiles"
- Who will generate these files and folders?
  - different applications will automatically create these to save the configurations and scripts to execute
  - operating system will create these specifically for the user (because we are on the single user's page), etc., .cache, .profile, based on the user's preference
- When creating the hidden files/folders, use "." as the start
